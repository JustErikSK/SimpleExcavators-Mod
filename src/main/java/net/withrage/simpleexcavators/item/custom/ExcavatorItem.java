...
public class ExcavatorItem extends Item {
    ...
    public boolean postMine(ItemStack stack, World world, BlockState state, BlockPos pos, LivingEntity miner) {
        ...
        if (!world.isClient() && miner instanceof PlayerEntity player) {
            ...
            mineExtraBlocks((ServerWorld) world, player, stack, pos, hitFace);
            }

        return result;
    }

    private void mineExtraBlocks(ServerWorld world,
                                 PlayerEntity player,
        ...
        if (face == Direction.UP || face == Direction.DOWN) {
            ...
            for (int dx = -1; dx <= 1; dx++) {
                for (int dy = -1; dy <= 1; dy++) {
                    result.add(origin.add(dx, dy, 0));
                }
            }
        } else if (face == Direction.EAST || face == Direction.WEST) {
            for (int dz = -1; dz <= 1; dz++) {
                for (int dy = -1; dy <= 1; dy++) {
                    result.add(origin.add(0, dy, dz));
                }
            }
        }
    ...
    private void breakOneExtraBlock(ServerWorld world,
                                    PlayerEntity player,
                                    ItemStack excavatorStack,
                                    BlockPos targetPos) {
        ...
        if (!player.isCreative()) {
            excavatorStack.damage(1, player);
        }
